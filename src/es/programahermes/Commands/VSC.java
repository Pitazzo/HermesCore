		...
		if(player instanceof Player){
			if(cmd.getName().equalsIgnoreCase("csv")){
				if(player.getInventory().contains(Material.NETHER_BRICK_ITEM)){
					...
					//player.sendMessage(ChatColor.GOLD+"Ox�geno: "+ChatColor.GREEN+MySQL.getOxygen(player.getName())+"L");
					player.sendMessage(ChatColor.GOLD+"Nivel de hidrataci�n: "+ChatColor.GREEN+sed+"%");
